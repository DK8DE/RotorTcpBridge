...
| `GETASELECT` / `SETASELECT` | `ACK_GETASELECT` / `ACK_SETASELECT` (bzw. `NAK_…`) — aktuell gewählte Antenne **1…3** lesen/schreiben. **Speicherort: AZ-Rotor-NVS** (Ziel = AZ-Slave-ID). PC‑Software fragt beim Connect per `GETASELECT` ab und schreibt beim Wechsel `SETASELECT` an den AZ-Rotor. Andere Master auf dem Bus (SETASELECT an AZ oder älter Broadcast DST 255) werden mitgeschnitten und in der UI übernommen. Ohne AZ-Rotor ist die Antennenauswahl deaktiviert. |
