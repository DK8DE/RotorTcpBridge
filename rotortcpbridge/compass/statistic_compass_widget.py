...
# EL: Firmware zeichnet immer 36 Bins (winkelunabhängig); UI verteilt sie auf 90° oder 180°.
EL_ARC_DEG = 90.0
EL_N_SEG = 36
...
EL_N_USED = EL_N_SEG - 2 * BINS_SKIP_EDGE  # 26
...
def paint_bins_heatmap_ring(
    ...
    el_arc_deg: float = EL_ARC_DEG,
) -> None:
    """5px Ring mit ACCBINS-Heatmap um einen Kompass.
    AZ: 62 nutzbare Bins (5–66) auf 360°.
    EL: immer 36 Firmware-Bins, auf 90° oder 180° gestreckt (nicht winkelabhängig).
    ...
    if elevation:
        ...
        if arc >= 180.0:
            ...
            arc = 90.0
        # Firmware: immer 36 Bins über den gesamten EL-Weg → auf den Anzeige-Bogen verteilen
        n_seg = EL_N_SEG
        n_used = max(1, n_seg - 2 * BINS_SKIP_EDGE)
        step = arc / n_used
        start_idx = BINS_SKIP_EDGE
        end_idx = n_seg - BINS_SKIP_EDGE
    else:
        n_used = AZ_N_USED
        step = 360.0 / n_used
        start_idx = BINS_SKIP_EDGE
...
class StatisticCompassWidget(QWidget):
    ...
    def set_elevation(self, on: bool) -> None:
        """Viertel-/Halbkreis-Modus (EL) ein/aus."""
        if self._elevation != on:
            self._elevation = bool(on)
            self.update()

    def set_el_max_deg(self, max_deg: float) -> None:
        """EL-Bogen: 90° oder 180° (GETROTORTYPE)."""
        try:
            mx = float(max_deg)
        except Exception:
            mx = EL_ARC_DEG
        mx = 180.0 if mx >= 180.0 else 90.0
        if abs(mx - self._el_max_deg) < 0.1:
            return
        self._el_max_deg = mx
        self.update()
