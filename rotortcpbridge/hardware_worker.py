...
class HardwareWorker:
    def __init__(self, cfg):
        self.cfg = cfg; self.q = queue.Queue(); self.running=False; self.sock=None; self.ser=None
    def start(self):
    ...
    def _loop(self):
        while self.running:
            if not self.sock and not self.ser: self._connect()
            try: line=self.q.get(timeout=0.2)
            except queue.Empty: continue
            try:
                data=(line+"\n").encode("ascii")
                if self.sock: self.sock.sendall(data)
                elif self.ser: self.ser.write(data)
