...
def _best_freq_recommendation(
    ...
    if dist_km < 80 and not los_blocked:
        if dist_km < 30:
            return dict(
                band="70 cm",
                freq_str="430–440 MHz",
                mode="Sichtverbindung",
                muf=None,
                color="#5cb85c",
            )
        return dict(
            band="2 m",
            freq_str="144–146 MHz",
            mode="VHF Sichtverbindung",
            muf=None,
            color="#5cb85c",
        )
    ...
    if dist_km <= 200:
        return dict(
            band="80 m",
            freq_str="3.5–3.8 MHz",
            mode="NVIS (F2-Schicht)",
            muf=round(muf, 1),
            color="#5cb85c",
        )
    if dist_km <= 500:
    ...
    return dict(band=name, freq_str=freq_str, mode=hop_str, muf=round(muf, 1), color=color)
...
class ElevationProfileWindow(QDialog):
    ...
    def __init__(
        ...
        self._live_preferred_when_online = bool(
            (cfg or {}).get("ui", {}).get("elevation_live_swpc", False)
        )
    ...
    def apply_internet_status(self, online: bool) -> None:
        ...
        if online:
            ...
            if self._cfg is not None:
                self._cfg.setdefault("ui", {})["elevation_live_swpc"] = (
                    self._live_preferred_when_online
                )
    ...
    def _on_data(self, elevations: List[float]) -> None:
        ...
        los = [round(los_start + (hn - los_start) * i / max(n - 1, 1), 1) for i in range(n)]
    ...
    def _on_error(self, msg: str) -> None:
        ...
        if sky:
            ...
            parts.append(
                f"<span style='opacity:0.55'>Ionosph. · {sky['band_name']}{nvis} · {fo}</span>"
            )
    ...
    def _rebuild_chart(self) -> None:
        ...
        self._view.setHtml(self._build_html(dists, elevations, los, h0, hn, diff))
    ...
    def _css_vars(self) -> dict:
        if self._dark:
            return dict(
                ...
                grid="#333333",
                elev_color="#4496eb",
                elev_fill="rgba(68,150,235,0.18)",
                los_color="#e07050",
    ...
    def _error_html(self, msg: str, show_internet_hint: bool = True) -> str:
        ...
        hint = (
            f"<div style='font-size:12px;opacity:0.6'>{t('elevation.internet_check')}</div>"
            if show_internet_hint
            else ""
        )
        return (
            f"<!DOCTYPE html><html><body style='margin:0;background:{v['bg']};"
    ...
    def _build_offline_html(self) -> str:
        ...
        if sky:
            ...
            skywave_section = (
                f'<div class="divider"></div><div class="cards">{mode_cards_html}</div>'
            )
        skywave_section = (
  ...
  <div class="card card-diff" style="border-left: 3px solid {qcol}; border-color:{qcol};"{_tt_rt}>
    ...
    <div class="card-value" style="font-size:13px;padding-top:3px;color:{qcol}">{_tr(diff["quality_key"])}</div>
        ...
        los_txt = (
            ...
            if blocked
            else f'<span style="color:#5cb85c">{_tr("elevation.los_free")}</span>'
        )
        ...
        muf_note = f"&nbsp;·&nbsp;MUF ≈ {best['muf']} MHz" if best.get("muf") else ""
        opt_label = (
            _tr("elevation.opt_band_live") if fo_f2 is not None else _tr("elevation.opt_band_mean")
        )
        _tt_opt = self._html_title_attr("elevation.tooltip_opt_band")
        opt_card = (
            f'<div class="card" style="border-color:{bc};border-width:2px"{_tt_opt}>'
        ...
        if skywave_section:
            ...
            skywave_section = f'<div class="divider"></div><div class="cards">{opt_card}</div>'
